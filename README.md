...
- 点【捕获元素…】自动打开这一步的网址，**主界面收起来**，抓完自动回来
- 页面平时照常能点能滚，**按住 Ctrl** 才是「我要抓」：划过画橙框、显示这个写法命中几个，按住 Ctrl 点一下就把 XPath 收下来（松开 Ctrl 就还给页面，Esc 取消）
- **浏览器不关**：下一次捕获直接接着用你当前停留的页面，不用重新打开、不用重新登录
- **重跑前面的节点**：按流程把当前这一步**之前**的节点在这个浏览器里真跑一遍，直接停在当前这一步该在的页面上（循环、条件、变量替换、步骤后等待全都跟正式跑一致）。问一次再跑，跑的时候能一键【跳过回放】
- 另外两个按钮：**重新加载**（等于 F5）、**回到入口页**（打开流程里第一个【打开网页】的地址）
- 【试运行】按钮：在还开着的浏览器里把这个元素跑一次（先圈出来，再点/填/选），结果直接贴在页面顶端 —— 定位对不对一眼就看出来
